...
vec3 getViewRefl(vec2 delta, float h_coe, vec3 facet_n_cs, inout float fresnel_sky, inout float ref_port_sun) {
    vec3 surface_n_cs = normalize(world_offset_n_cs - cos((fragTexCoord.x + delta.x) / wave_h_domain + ubo.time) * h_coe * lat_n_cs - cos((fragTexCoord.y + delta.y) / wave_h_domain + ubo.time) * h_coe * lng_n_cs);
    ...
    if (dot(n_view_refl, world_offset_n_cs) <= 0) {
        vec3 tmp = dot(-vertex_pos_n_cs, world_offset_n_cs) * world_offset_n_cs;
        n_view_refl = normalize(vertex_pos_n_cs + tmp);
    }
    return n_view_refl;
}
...
void main() {
    if (ubo.target == TARGET_SKY) {
        ...
        outColor.rgb = scatter_rgba.rgb + (1 - scatter_rgba.a) * (
            scatter_rgba_refl.rgb * fresnel_sky + (1 - fresnel_sky) * water_refrac * scatter_rgba_refl.rgb + sun_color * scatter_rgba_refl.a * ref_port_sun
        );
    } else {
